...
def process_file_local(filename: str) -> str:
...
def add_audio(history: list, audio: str) -> tuple:
    ...
    try:
        ...
        transcription = transcribe_audio_openai(stable_audio_path)
...
def add_file_db(file) -> gr.Dropdown:
    ...
    if metadata_service is not None:
        try:
            ...
            print(f"⚠️ Pipeline process_file failed, fallback local: {e}")

    doc_name = process_file_local(file.name)
...
def add_audio_file_db_local(audio_file) -> gr.Dropdown:
    """
...
def add_audio_file_db(audio_file) -> gr.Dropdown:
    if metadata_service is not None:
        try:
            ...
            print(f"⚠️ Pipeline add_audio_file_db failed, fallback local: {e}")

    return add_audio_file_db_local(audio_file)
...
def bot(history: list, modality: str, selected_note: str):
    ...
    try:
        if isinstance(user_input, str):
            ...
            query = transcribe_audio_openai(user_input)
...
def main():
    ...
    try:
        ...
        print(f"⚠️ MetadataService indisponible, fallback local: {e}")
        metadata_service = None
